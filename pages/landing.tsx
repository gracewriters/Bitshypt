...
const tickerItems = [
  ...
  { label: 'Avg ZK-Proof Time', value: '4.2s' },
  ...
  { label: 'NFTs', value: 'Electricity-Backed', highlight: true },
...
const Landing: NextPage<LandingProps> = ({ bodyHtml, pageStyles }) => {
  ...
  const handleMintNft = () => {
    ...
    const nftId = 'NFT-' + Math.random().toString(16).slice(2, 10).toUpperCase();
    const zk = Array.from({ length: 32 }, () => Math.floor(Math.random() * 16).toString(16)).join('');
    setMintForm((prev) => ({ ...prev, trackId: nftId }));
    setMintOutput(
      `✅ Package NFT Minted!\nNFT ID: ${nftId}\nZK Proof: ${zk.slice(0, 24)}...\nSender: ${sender.slice(0, 32)}\nReceiver: ${receiver.slice(0, 32)}\nMetadata: ${meta.slice(0, 32)}\n+5.0 $BS block reward credited`
    );
  };
  ...
  const transformNode = (domNode: any) => {
    ...
    if (onclick === 'mintNFA()' || onclick === 'mintNFT()') {
      return (
        <button className={className} onClick={handleMintNft}>
          {children}
        </button>
      );
    }
    ...
    if (onclick === 'confirmPoD()' || onclick === 'confirmZK()') {
      return (
        <button className={className} onClick={handleConfirmZk}>
          {children}
        </button>
      );
    }
    ...
    if (id === 'hA' || id === 'vA') {
      return <span id={id} suppressHydrationWarning>{Math.round(demoTelemetry.alt)}</span>;
    }

    if (id === 'hS') {
      return <span id="hS" suppressHydrationWarning>{demoTelemetry.speed.toFixed(1)}</span>;
    }

    if (id === 'hB') {
      return <span id="hB" suppressHydrationWarning>{Math.round(demoTelemetry.battery)}</span>;
    }

    if (id === 'hV') {
      return (
        <div id="hV" suppressHydrationWarning style={{ fontSize: 10, color: demoTelemetry.vlos === 'OK' ? 'var(--teal)' : 'var(--blue)' }}>
          {demoTelemetry.vlos}
        </div>
      );
    }
    ...
    if (id === 'hH') {
      return <span id="hH" suppressHydrationWarning>{demoTelemetry.hash}</span>;
    }

    if (id === 'vD') {
      return <span id="vD" suppressHydrationWarning>{demoTelemetry.distance}</span>;
    }

    if (id === 'vV') {
      return <span id="vV" suppressHydrationWarning>{demoTelemetry.visibility}</span>;
    }

    if (id === 'vC') {
      return <span id="vC" suppressHydrationWarning>{demoTelemetry.cvScore}</span>;
    }

    if (id === 'pA') {
      return <div id="pA" suppressHydrationWarning>{Math.round(demoTelemetry.alt)}m</div>;
    }

    if (id === 'pS') {
      return <div id="pS" suppressHydrationWarning>{Math.round(demoTelemetry.speed)}m/s</div>;
    }

    if (id === 'pB') {
      return <div id="pB" suppressHydrationWarning>{Math.round(demoTelemetry.battery)}%</div>;
    }

    if (id === 'zkP') {
      return <div id="zkP" suppressHydrationWarning>{proofHash}</div>;
    }
    ...
    // Default fallback: sanitize any leftover lowercase event handler attributes
    // (e.g. `onclick`, `onmouseover`) which cause React warnings, then let the
    // parser handle the node.
    try {
      Object.keys(attribs).forEach((k) => {
        if (/^on[a-z]/.test(k)) {
          // remove legacy inline handlers left over in the static HTML
          delete attribs[k];
        }
      });
    } catch (e) {
  ...
  const parsedBody = useMemo(() => {
    // Remove leftover inline lowercase event handlers (onclick, onmouseover, etc.)
    // from the static HTML to avoid React invalid event handler warnings.
    const sanitized = bodyHtml
      .replace(/\s+on[a-z]+\s*=\s*"[^"]*"/g, '')
      .replace(/\s+on[a-z]+\s*=\s*'[^']*'/g, '');
    return parse(sanitized, { replace: transformNode });
  }, [
    bodyHtml,
    mobileNavOpen,
    dropdownOpen,
  ...
  return (
    <>
      ...
      <div suppressHydrationWarning>{mounted ? parsedBody : null}</div>
    </>
  );
};
